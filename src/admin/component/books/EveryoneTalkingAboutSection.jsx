...
// Modal Component
function AddTalkingAboutModal({ isOpen, onClose, onAdd }) {
  const [newTalkingAbout, setNewTalkingAbout] = useState({
    heading: "",
    description: "",
    image: null,
  });
  ...
  const handleImageUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      setNewTalkingAbout({ ...newTalkingAbout, image: file });
      const reader = new FileReader();
      reader.onload = (e) => setImagePreview(e.target.result);
      reader.readAsDataURL(file);
    }
  ...
  const handleSubmit = () => {
    if (!newTalkingAbout.heading) {
      alert("Please enter a talking about heading");
      return;
    }
    onAdd(newTalkingAbout);
    // Reset form
    setNewTalkingAbout({ heading: "", description: "", image: null });
    setImagePreview(null);
    onClose();
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              value={newTalkingAbout.heading}
              onChange={(e) =>
                setNewTalkingAbout({ ...newTalkingAbout, heading: e.target.value })
              }
              className="w-full px-4 py-3 focus:outline-none border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              type="text"
              placeholder="Enter Talking About"
              value={newTalkingAbout.heading}
              onChange={(e) =>
                setNewTalkingAbout({ ...newTalkingAbout, heading: e.target.value })
              }
...
// Design after adding the talking about from the modal
function TalkingAboutCard({ talkingAbout, onDelete }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-300">
      <div className="flex items-start space-x-5">
        {/* Image */}
        {talkingAbout.image && (
          <img
            src={
              typeof talkingAbout.image === "string"
                ? talkingAbout.image
                : URL.createObjectURL(talkingAbout.image)
            }
            alt={talkingAbout.heading}
            className="w-28 h-28 object-cover rounded-lg border shadow-sm"
          />
        )}
        ...
        <div className="flex-1">
          <h3 className="text-xl font-bold text-gray-800 leading-snug">
            {talkingAbout.heading}
          </h3>
          <p className="text-gray-600 text-sm mt-2 leading-relaxed">
            {talkingAbout.description}
          </p>
        </div>

        {/* Delete Button */}
        <button
          onClick={() => onDelete(talkingAbout.id)}
          className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-300 text-sm shadow-sm"
        >
          Delete
        </button>
...
// TalkingAboutSection Component
function TalkingAboutSection({
  talkingAbout,
  onUpdate,
  onDelete,
  onOpenModal,
  ...
  const [isExpanded, setIsExpanded] = useState(false);
  const [imagePreview, setImagePreview] = useState(
    talkingAbout.image ? URL.createObjectURL(talkingAbout.image) : null
  );

  const handleImageUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      onUpdate(talkingAbout.id, "image", file);
      const reader = new FileReader();
      reader.onload = (e) => setImagePreview(e.target.result);
      reader.readAsDataURL(file);
    }
  ...
  const handleInputChange = (field, value) => {
    onUpdate(talkingAbout.id, field, value);
  };

  const handleReset = () => {
    onUpdate(talkingAbout.id, "heading", "");
    onUpdate(talkingAbout.id, "description", "");
    onUpdate(talkingAbout.id, "image", null);
    setImagePreview(null);
  };

  const handleSubmit = () => {
    console.log("Talking About Data:", talkingAbout);
    alert("Talking About saved successfully!");
  };
  ...
  return (
    <div className="w-full max-w-6xl  mt-4 mx-auto bg-white border border-gray-200 rounded-lg shadow-sm">
      <div>
        ...
        >
          <h2 className="text-lg font-semibold text-gray-800">
            {talkingAbout.heading || "Talking About Management"}
          </h2>
          <div className="flex items-center space-x-2">
            {isExpanded ? (
              <ChevronUp className="w-5 h-5 text-gray-500" />
        ...
        {!isExpanded && (
          <div className="p-4 flex justify-between items-center">
            <span className="text-gray-700">
              {talkingAbout.heading || "No Heading"}
            </span>
            <button
              onClick={(e) => {
                e.stopPropagation();
        ...
        {isExpanded && (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">
                  Main Talking About Heading
                </label>
                <input
                  type="text"
                  placeholder="Enter Main Heading"
                  value={talkingAbout.heading}
                  onChange={(e) => handleInputChange("heading", e.target.value)}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus: outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            ...
            {/* Display Added Talking About */}
            {addedTalkingAbout && addedTalkingAbout.length > 0 && (
              <div className="mt-6 space-y-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  Added Talking About ({addedTalkingAbout.length})
                </h3>
                <div className="space-y-3">
                  {addedTalkingAbout.map((ta) => (
                    <TalkingAboutCard
                      key={ta.id}
                      talkingAbout={ta}
                      onDelete={onDelete}
...
// Main Component
export default function EveryoneTalkingAboutSection() {
  const [talkingAboutList, setTalkingAboutList] = useState([
    {
      id: 1,
      heading: "Everyone Talking About Management",
      description: "Explore our collection",
      image: null,
    },
  ]);
  const [addedTalkingAbout, setAddedTalkingAbout] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const updateTalkingAbout = (id, field, value) => {
    setTalkingAboutList(
      talkingAboutList.map((ta) =>
        ta.id === id ? { ...ta, [field]: value } : ta
      )
    );
  };

  const addNewTalkingAbout = (newTalkingAboutData) => {
    const newId =
      Math.max(
        ...talkingAboutList.map((t) => t.id),
        ...addedTalkingAbout.map((t) => t.id),
        0
      ) + 1;
    setAddedTalkingAbout([...addedTalkingAbout, { id: newId, ...newTalkingAboutData }]);
  };

  const deleteTalkingAbout = (id) => {
    if (window.confirm("Are you sure you want to delete this?")) {
      setAddedTalkingAbout(addedTalkingAbout.filter((t) => t.id !== id));
    }
  };
    ...
    <div className="p-6">
      ...
      {talkingAboutList?.map((ta) => (
        <TalkingAboutSection
          key={ta.id}
          talkingAbout={ta}
          onUpdate={updateTalkingAbout}
          onDelete={deleteTalkingAbout}
          onOpenModal={() => setIsModalOpen(true)}
          addedTalkingAbout={addedTalkingAbout}
        />
      ))}

      {/* Add Talking About Modal */}
      <AddTalkingAboutModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onAdd={addNewTalkingAbout}
      />
    </div>
  );
}
